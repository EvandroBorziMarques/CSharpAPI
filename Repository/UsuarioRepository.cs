...
namespace Cadastro.Repository
{
    public class UsuarioRepository : DapperRepository<Usuario>, IUsuarioRepository
    {
        ...
        {
            using var dbConnection = new SqlConnection(ConnectionString);
            var query = @"SELECT Usuario.Id, Usuario.Nome, Pedido.Id, Pedido.NomeProduto, Pedido.UsuarioId FROM Usuario
                LEFT JOIN Pedido ON Usuario.Id = Pedido.UsuarioId Where Usuario.Id = @Id";
